...
# =========================
# YoY feature engineering (for forecasting)
# =========================
tfr_features = merged[["Year", "Total_Fertility_Rate"]].sort_values("Year").copy()
...
X_yoy = tfr_features[["Time_Index"]]
y_yoy = tfr_features["TFR_Annual_Change"]

model_yoy = LinearRegression()
model_yoy.fit(X_yoy, y_yoy)
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Trends", "Housing vs fertility", "Model evaluation", "Predictions (YoY)"]
)
...
with tab4:
    st.subheader("TFR Forecast using YoY Change (2025–2030)")

    last_row = tfr_features.iloc[-1]
    last_tfr = last_row["Total_Fertility_Rate"]

    # Base YoY = recent average
    base_yoy = tfr_features.tail(5)["TFR_Annual_Change"].mean()

    forecast_years = range(2025, 2031)
    rows = []

    decay = 0.90  # 10% reduction in decline each year

    for i, year in enumerate(forecast_years):
        yoy_change = base_yoy * (decay**i)
        last_tfr = last_tfr + yoy_change

        rows.append(
            {
                "Year": year,
                "Predicted_TFR": last_tfr,
                "Predicted_Annual_Change": yoy_change,
            }
        )
    ...
    fig_forecast = px.line(
        ...
        y="Total_Fertility_Rate",
        markers=True,
        title="Actual vs YoY-based TFR Projection (Decaying Momentum)",
    )

    fig_forecast.add_scatter(
        x=forecast_df["Year"],
        y=forecast_df["Predicted_TFR"],
        mode="lines+markers",
        name="Projected TFR",
        line=dict(dash="dash"),
    )

    fig_forecast.add_vline(x=2024.5, line_dash="dot")
